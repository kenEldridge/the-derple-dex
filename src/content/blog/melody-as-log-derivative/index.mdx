...
*Top: piano keys space octaves evenly, while a linear Hz axis does not. Bottom: the cochlea behaves similarly at first approximation: equal octave steps map to roughly equal membrane distances, while the Hz range doubles each octave (orange curve).*
...
*The cochlea, uncoiled. The basilar membrane widens from base (narrow, stiff, high-frequency) to apex (wide, more compliant, low-frequency). The labels suggest an approximately log-like map. Diagram by Kern et al., [CC BY 2.5](https://creativecommons.org/licenses/by/2.5/).*
